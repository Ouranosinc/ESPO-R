    ...
    for k, v in chunks.items():
        if isinstance(v, dict):
            ...
            if isinstance(v, str) and v.endswith('year'):
                ...
                Nt = n * {'noleap': 365, '360_day': 360, 'all_leap': 366}.get(calendar, 365.25)
                chunks[k] = int(Nt)
            elif v == -1:
                chunks[k] = timesize
    return chunks
...
@parse_config
def stack_drop_nans(ds, mask, *, new_dim='loc', to_file=None):
...
@parse_config
def unstack_fill_nan(ds, *, dim='loc', coords=None):
    ...
    if isinstance(coords, (list, tuple)):
        ...
        dims, crds = zip(*[(name, crd.load().values) for name, crd in ds.coords.items() if crd.dims == (dim,)])

    out = ds.drop_vars(dims).assign_coords({dim: pd.MultiIndex.from_arrays(crds, names=dims)}).unstack(dim)
